...
/* menambahkan elemen pertama dengan nilai val jika alokasi berhasil */
    ...
    if (p != NULL){
        if (isEmpty(*l)){
            ...
            last = FIRST(*l);
            while (NEXT(last) != FIRST(*l)){ // quits loop when last el is acquired
                last = NEXT(last);
            }
            NEXT(p) = FIRST(*l);
            NEXT(last) = p;
